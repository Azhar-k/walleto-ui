...
class _RecurringPaymentFormScreenState extends ConsumerState<RecurringPaymentFormScreen> {
  ...
  void initState() {
    super.initState();
    _nameController = TextEditingController(text: widget.existingPayment?.name ?? '');
    _amountController = TextEditingController(text: widget.existingPayment?.amount.toString() ?? '');
    _dueDayController = TextEditingController(text: widget.existingPayment?.dueDay.toString() ?? '');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.existingPayment == null ? 'Add Recurring Payment' : 'Edit Payment'),
      ),
      body: _isLoading 
              ...
              child: Column(
                ...
                children: [
                  TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Payment Name *', prefixIcon: Icon(Icons.receipt)),
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Due Day (1-31) *', prefixIcon: Icon(Icons.calendar_today)),
                    keyboardType: TextInputType.number,
                    validator: (v) {
                       if (v == null || v.isEmpty) return 'Required';
                       final val = int.tryParse(v);
                       if (val == null || val < 1 || val > 31) return 'Must be a valid day (1-31)';
                       return null;
                    },
                  ),
                  const SizedBox(height: 16),
                  ListTile(
                    ...
                    subtitle: Text(_expiryDate != null ? DateFormat.yMMMd().format(_expiryDate!) : 'Select Date'),
