...
class _SelfTransferScreenState extends ConsumerState<SelfTransferScreen> {
  ...
  void _submit() async {
    if (_formKey.currentState!.validate() && _fromAccount != null && _toAccount != null && _category != null) {
      ...
      final dateTime = DateTime(
        _transferDate.year, _transferDate.month, _transferDate.day,
        _transferTime.hour, _transferTime.minute
      );
      ...
      try {
       ...
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please fill all mandatory dropdowns.')));
    }
  }
                      ...
                      child: Column(
                        ...
                        children: [
                          DropdownButtonFormField<Account>(
                            ...
                            decoration: const InputDecoration(labelText: 'From Account *', prefixIcon: Icon(Icons.arrow_upward, color: Colors.red)),
                          ...
                          DropdownButtonFormField<Account>(
                            ...
                            decoration: const InputDecoration(labelText: 'To Account *', prefixIcon: Icon(Icons.arrow_downward, color: Colors.green)),
                            items: accounts.map((a) => DropdownMenuItem(value: a, child: Text(a.accountName))).toList(),
                            ...
                            children: [
                              Expanded(
                                child: ListTile(
                                  ...
                                  subtitle: Text(DateFormat.yMMMd().format(_transferDate)),
                                  leading: const Icon(Icons.calendar_today),
                                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8), side: BorderSide(color: Colors.grey.shade400)),
                                  onTap: () async {
                                    final picked = await showDatePicker(context: context, initialDate: _transferDate, firstDate: DateTime(2000), lastDate: DateTime(2100));
                              ...
                              Expanded(
                                child: ListTile(
                                  ...
                                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8), side: BorderSide(color: Colors.grey.shade400)),
                                  onTap: () async {
                                    final picked = await showTimePicker(context: context, initialTime: _transferTime);
                          ...
                          DropdownButtonFormField<Category>(
                            ...
                            items: expenses.map((c) => DropdownMenuItem(value: c, child: Text(c.name))).toList(),
                          ...
                          ElevatedButton(
                            ...
                            child: _isLoading 
                                ...
                                : const Text('TRANSFER FUNDS', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          )
                        ],
                      ),
                    ),
